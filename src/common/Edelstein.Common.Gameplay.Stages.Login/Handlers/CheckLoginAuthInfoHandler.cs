...
{
    private readonly IPipeline<IAuthLoginBasic> _pipeline;

    public CheckLoginAuthInfoHandler(IPipeline<IAuthLoginBasic> pipeline) => _pipeline = pipeline;
    ...
    public override Task Handle(ILoginStageUser user, IPacketReader reader)
    {
        ...
        return _pipeline.Process(new AuthLoginBasic(user, username, password));
    }
}
